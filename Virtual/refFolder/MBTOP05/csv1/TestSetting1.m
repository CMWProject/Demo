...
try
    ...
    cd('Z:\ModelVariation\Bus\Virtual\resFolder\MBTOP05\csv1');
    load_system('Z:\ModelVariation\Bus\Virtual\resFolder\MBTOP05\csv1\tarmodel');
    Simulink.data.evalinGlobal('tarmodel', 'load(''Z:\ModelVariation\Bus\Virtual\resFolder\MBTOP01\busObj.mat'')');
    ...
    % コンフィグレーションパラメータ設定
    set_param(cmwOption.modelHandle, 'SignalLogging', 'on');                        % 信号ログ設定
    set_param(cmwOption.modelHandle, 'SignalLoggingName', 'logsout');               % 信号ログ設定
...
% 入力データ
cmwOption.data{1} = struct('signal1', timeseries(double(MBTOP.OP5.castStrToNum([{'1';'2';'3';}], 'double')), cmwOption.time),'signal2', timeseries(double(MBTOP.OP5.castStrToNum([{'4';'5';'6';}], 'double')), cmwOption.time));
